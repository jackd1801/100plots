...
plot3 <- ggplot(tidydoses)+
  ...
  labs(x="",
       y="",
       title = "Daily COVID-19 vaccine doses administered - UK",
       subtitle="Dec 2020 - Feb 2022",
       caption="Data from coronavirus.data.gov\nProduced by @JackD1801",
       colour="Age Group",
       linetype="Dose Number")+ 
  scale_x_date(date_labels = "%b-%y")+
  scale_y_continuous(expand=c(0,0), labels = label_comma())+
  dark_theme_classic()+
...
plot3.animation = plot3 + 
  transition_reveal(Date)+
  view_follow(fixed_x=FALSE, fixed_y=FALSE)

#plot3.animation

animate(plot3.animation, height=500, width=800, fps=30, duration = 25, 
        end_pause=60, res=100)

anim_save("./Plot4DailyVaccines/output/Plot4Final.gif")
